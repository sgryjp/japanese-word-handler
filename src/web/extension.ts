...
function _select(editor: TextEditor, wordSeparators: string, find: Function) {
  editor.selections = editor.selections.map(
    (s) =>
      new Selection(s.anchor, find(editor.document, s.active, wordSeparators)),
  );
  if (editor.selections.length === 1) {
    editor.revealRange(editor.selection);
  }
  ...
  return editor
    .edit((e) => {
      ...
      let selections = editor.selections.map(
        (s) =>
          new Selection(s.anchor, find(document, s.active, wordSeparators)),
      );
...
function findPreviousWordStart(
  ...
  function prevCharIsWhitespace() {
    ...
    return (
      classify(doc, prevPos.line, prevPos.character) === CharClass.Whitespace
    );
  }
  let pos = caretPos;
